...
public class EnhancedXBoxController extends Joystick {
	...
	ButtonTypes[] buttonMode;//Each element is the emulated type of the corresponding button.
	
	/**
	 * Creates a new Enhanced Controller on the specified port.
	 * 
	 * @param port Int containing the port number of the controller
	 */
	 ...
	 * 
	 * @param button The affected XBoxButtons button
	 * @param mode The new ButtonTypes type
	 */
	 ...
	 * 
	 * @param axis The affected axis
	 * @param dead Double containing the new deadband value.  Must be greater
	 ...
	 * type of the buttons.
	 * 
	 * @param button The button whose value should be read 
	...
	/**
	 * Reads the value of the specified axis after deadbanding.  The 
	 * deadbanded values are linearly mapped to new values such that at the 
	 * deadband, the value is 0, but at full power the value is 1.  This
	 * prevents jerky readings when the raw value hovers around the deadband.
	 * <p>
	...
	public double getValue(XBoxAxes axis) {
		 ...
		 * 0.0, but after that it begins increasing from 0.0, but with a steeper sloping line so that at the deadband it is 0.0, but at 
		 ...
		 */
		double raw = 0.0;
		if (axis == XBoxAxes.TRIGGER) {							//Emulates previous years in which both triggers were one one axis
		...
		if (raw <= 0) {
			adjustedValue = (1/(1-dead))*raw + (dead/(1-dead));//-(((Math.abs(raw) - 1) / (1 - dead)) + 1);
		} else {
			adjustedValue = (1/(1-dead))*raw - (dead/(1-dead));//((raw - 1) / (1 - dead)) + 1;
		}
	 ...
	 * 
	 * @param pad The joypad to read
	 * @return Int containing the angle of the angle of the joypad.  It will
	 * be a multiple of 45 degrees representing the position that was pressed.
	 * -1 will be returned if nothing was pressed.
	 */
	...
	/**
	 * Reads the raw value of the specified button.  This method does not
	 * respect emulated types.
	 * 
	...
	/**
	 * Reads the raw value of the specified axis.  This method does not
	 * deadband values.
	 * 
	 * @param axis The axis to read
	...
	public double getRawValue(XBoxAxes axis) {
		//Same as calling joystick.getRawAxis(number), but is overloaded and used the XBoxAxes enum
		return super.getRawAxis(axis.getValue());
	}
	
	/**
	 * Reads and debounces the specified button.  When read with this method,
	 * the button will read high only on the rising edge, regardless of the
	 * emulated type of the button.
	 * 
	 * @param button The button to read
